package ie.wit.studentshare.ui.auth
...
import ie.wit.studentshare.databinding.LoginBinding
import ie.wit.studentshare.ui.nav.Navigation
...
class Login : AppCompatActivity() {
    ...
    private fun validateForm(): Boolean {
        ...
        if (TextUtils.isEmpty(email)) {
            loginBinding.fieldEmailBox.error = "Required field"
            valid = false
        } else if (!loginBinding.fieldEmail.text.toString().matches(emailRegex)) {
            loginBinding.fieldEmailBox.error = "Enter valid email format"
            valid = false
        } else {
            loginBinding.fieldEmailBox.error = null
        }

        val password = loginBinding.fieldPassword.text.toString()
        if (TextUtils.isEmpty(password)) {
            loginBinding.fieldPasswordBox.error = "Required field"
            valid = false
        } else if (loginBinding.fieldPassword.length() < 6){
            loginBinding.fieldPasswordBox.error = "Password should be at least 6 characters"
            valid = false
        } else {
            loginBinding.fieldPasswordBox.error = null
        }
        return valid
    }
}
